...
__attribute__((used, section(".limine_requests_start"), aligned(8))) volatile uint64_t requests_start_marker[] =
    LIMINE_REQUESTS_START_MARKER;

__attribute__((used, section(".limine_requests"), aligned(8))) volatile uint64_t base_revision[] =
    LIMINE_BASE_REVISION(6);

__attribute__((used, section(".limine_requests"), aligned(8))) volatile limine_bootloader_info_request bootloader_info_request = {
    .id = LIMINE_BOOTLOADER_INFO_REQUEST_ID,
    .revision = 0,
    .response = nullptr,
};

__attribute__((used, section(".limine_requests"), aligned(8))) volatile limine_firmware_type_request firmware_type_request = {
    .id = LIMINE_FIRMWARE_TYPE_REQUEST_ID,
    .revision = 0,
    .response = nullptr,
};

__attribute__((used, section(".limine_requests"), aligned(8))) volatile limine_stack_size_request stack_size_request = {
...
__attribute__((used, section(".limine_requests"), aligned(8))) volatile limine_memmap_request memory_map_request = {
    .id = LIMINE_MEMMAP_REQUEST_ID,
    .revision = 0,
    .response = nullptr,
};

__attribute__((used, section(".limine_requests_end"), aligned(8))) volatile uint64_t requests_end_marker[] = LIMINE_REQUESTS_END_MARKER;
