...
TEST(TerminalRepaintStateTest, NonBatchScrollRequestsImmediateFullRepaint)
{
    ...
    EXPECT_FALSE(state.pending_full_repaint());
}

TEST(TerminalRepaintStateTest, BatchScrollDefersFullRepaint)
{
    kernel::display::TerminalRepaintState state;
    ...
    EXPECT_FALSE(request.repaint_text_layer);
    EXPECT_FALSE(request.repaint_higher_layers);
    EXPECT_TRUE(state.pending_full_repaint());
...
TEST(TerminalRepaintStateTest, PendingFullRepaintDominatesSmallerDirtyRects)
{
    kernel::display::TerminalRepaintState state;
    EXPECT_TRUE(state.begin_batch());
